...
export function CartPage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      setCouponError(err?.response?.data?.message ?? 'کد تخفیف معتبر نیست');
    } finally {
            ...
            <div key={item.id} className="bg-white border border-border rounded-card p-4 flex gap-4">
              {item.product?.image && (
                <Link to={`/products/${item.product.slug}`}>
                  <img src={import.meta.env.VITE_API_BASE_URL?.replace('/api', '') + item.product.image} alt={item.product.name} className="w-24 h-24 rounded-button object-cover" />
                </Link>
              )}
              ...
              <div className="flex flex-col items-end justify-between gap-2">
                <button onClick={() => removeItem(item.id)} className="text-text-muted hover:text-danger transition-colors">
                  <Trash2 size={16} />
                </button>
                <QuantitySelector
              ...
              <div>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 bg-gold-light text-gold-dark rounded-button text-sm font-semibold hover:bg-gold hover:text-text-primary transition-colors"
                  >
                    <Tag size={16} />
                  </button>
                </div>
